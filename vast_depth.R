...
df = df %>%
  ...
  select(year, station, depth, lon, lat, raw_N, swept_area, tag, depth) %>% na.omit()
summary(df)
...
# 2.1 Data-frame
head(df)
Data_Geostat = df %>% select(year, lon, lat, raw_N, swept_area, depth) %>% mutate(Catch_KG = raw_N/swept_area) %>% dplyr::rename(Year = year, Lon = lon, Lat = lat, Depth = depth)
...
TmbData = make_data(
  ...
  Q_ik = matrix(Data_Geostat[, "Depth"], ncol = 1),
...
vast_output_dirname = "/Users/Yuki/Dropbox/sokouo1/make_VASTdata/vast2021-07-05_dens_lognorm100"
fig_output_dirname = "/Users/Yuki/Dropbox/sokouo1/make_VASTdata/vast2021-07-05_dens_lognorm100"
