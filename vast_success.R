...
df = df %>%
  ...
  select(year, station, depth, lon, lat, raw_N, swept_area, tag) %>% na.omit()
summary(df)
...
# 2.1 Data-frame
head(df)
Data_Geostat = df %>% select(year, lon, lat, raw_N, swept_area) %>% mutate(Catch_KG = raw_N/swept_area) %>% dplyr::rename(Year = year, Lon = lon, Lat = lat)
...
vast_output_dirname = "/Users/Yuki/Dropbox/sokouo1/make_VASTdata/vast2020-12-28_dens_lognorm100"
fig_output_dirname = "/Users/Yuki/Dropbox/sokouo1/make_VASTdata/vast2020-12-28_dens_lognorm100"
