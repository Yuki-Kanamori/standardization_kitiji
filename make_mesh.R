  ...
  function(loc_x, loc_g, loc_i, Method, Extrapolation_List, anisotropic_mesh=NULL, fine_scale=FALSE, ...){
    ...
    if( is.null(anisotropic_mesh)){
      if( fine_scale==FALSE ){
        ...
        anisotropic_mesh = INLA::inla.mesh.create( loc_x, plot.delay=NULL) #ここのoutputsはbarrier modelと同じ
      }else{
    ...
    plot(anisotropic_mesh) #meshが良くない
    
    anisotropic_spde = INLA::inla.spde2.matern(anisotropic_mesh, alpha=2) #barrier modelのoutputsと違う
    ...
    barrier_finite_elements = INLA:::inla.barrier.fem(mesh=anisotropic_mesh,
                                                      barrier.triangles=anisotropic_mesh_triangles_over_land) #inla.barrier.fem is an internal method producing the Finite Element matrices.
